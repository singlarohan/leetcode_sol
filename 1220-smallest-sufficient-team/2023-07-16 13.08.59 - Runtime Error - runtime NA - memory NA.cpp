...
public:
    ...
    long long solve(int mask, int i, long long pmask, vector<string>& skills, vector<vector<string>>& people){
        ...
        if(newMask != mask)
            take = solve(newMask, i + 1, pmask | ((long long) 1 << i), skills, people);
